...
// 			m_ps.startDelay		+= m_fStartDelayTime;
			...
			if (ren != null)
			{
                float fRenderLength = ren.lengthScale;
                ren.lengthScale = fRenderLength * m_fRenderLengthRate;
...
//        else Log.infoWarning(info.Name + " could not be write.");
   ...
   //     else Log.infoWarning(info.Name + " could not be read.");
	...
	public static void SetPropertyValue(SerializedObject serObj, string propertyName, object value, bool bUpdate)
	{
		...
		if (sp == null)
		{
 //           Logger.LogWarning("SetPropertyValue error - " + propertyName);
			return;
		}
		...
		if (bUpdate)
		{
 			...
 			{
//				StarEngine.Debuger.LogWarning("Failed - ApplyModifiedProperties " + propertyName);		// dup
			}
		}
	}
...
// 			default: Log.info("Undefile propertyType - SetPropertyValue " + sp.name); break;
		}
	}
