...
public class NcEffectBehaviour : MonoBehaviour
{
    ...
    protected bool IsCreatingEditObject()
    {
        ...
        if (effroot == null)
            return false;
        return (effroot.transform.childCount == 0);//effroot.transform.GetChildCount()
    }
    ...
    {
        for (int n = parent.transform.childCount -1; 0 <= n; n--) // parent.transform.GetChildCount()
        {
            if (n < parent.transform.childCount)//parent.transform.GetChildCount())
            {
    ...
    static Texture[] PreloadPrefab(GameObject tarObj, List<GameObject> parentPrefabList, bool bCheckDup)
    {
        if (parentPrefabList.Contains(tarObj))
        {
            if (bCheckDup)
            {
                ...
                //             Log.infoWarning("LoadError : Recursive Prefab - " + str + tarObj.name);
                return null;	// error
            }
            else return null;
        }
    ...
    //
    public virtual void RecordStartState()
    {
        m_startActiv = gameObject.activeInHierarchy;
    }
    public virtual void ResetToStartState()
    {
        gameObject.SetActive(m_startActiv);
    }
    // �Ƿ���ս����ʹ��(���ڼ���FX���Զ����ٺͶ���ص����ó�ͻ)
