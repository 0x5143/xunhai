...
Shader "Blue/Character/BaseCube" 
{
	Properties 
	{
		...
		Pass 
		{
			...
			fixed4 frag_surf (VertexOutput IN) : COLOR 
			{
				IN.normal = normalize(IN.normal);
			  	...
			  	finalColor.rgb += texColor * control.r * (_SpecularColor.rgb*8*spec*pow(spec , _SpecularPow)+min(_SpecularPow/10,0.3)+IN.vlight);
			  	finalColor.rgb += control.b *texCUBE (_CubeTex, IN.worldRefl)*_CubeValue;
